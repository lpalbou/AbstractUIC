...
function normalizeSettings(raw) {
    ...
    return {
        ...
        header_density: typeof o.header_density === "string" && o.header_density ? o.header_density : APPEARANCE_DEFAULTS.header_density,
    };
}
export function appearanceStorageKey(appId) {
 ...
 */
export function useAppearanceSettings(appId, options) {
    const key = appearanceStorageKey(appId);
    const defaults = useMemo(() => ({ ...APPEARANCE_DEFAULTS, ...(options?.defaults || {}) }), 
    ...
    const [settings, setSettings] = useState(() => {
        ...
        try {
            ...
            if (raw)
                initial = normalizeSettings({ ...defaults, ...JSON.parse(raw) });
        }
        catch {
...
export function AfAppearanceDialog(props) {
    useEffect(() => {
        if (!props.open)
            return;
        const onKey = (e) => {
    ...
    if (!props.open)
        return null;
    return (_jsx("div", { className: "af-appearance-overlay", onClick: props.onClose, role: "presentation", children: _jsxs("div", { className: "af-appearance", role: "dialog", "aria-modal": "true", "aria-label": props.title || "Appearance", onClick: (e) => e.stopPropagation(), children: [_jsx("div", { className: "af-appearance__title", children: props.title || "Appearance" }), _jsx("div", { className: "af-appearance__note", children: props.note || "Stored locally in this browser, per app." }), _jsxs("div", { className: "af-appearance__grid", children: [_jsx("label", { className: "af-appearance__label", children: "Theme" }), _jsx(ThemeSelect, { value: props.value.theme, onChange: (theme) => props.onChange({ ...props.value, theme }) }), _jsx("label", { className: "af-appearance__label", children: "Font size" }), _jsx(FontScaleSelect, { value: props.value.font_scale, onChange: (font_scale) => props.onChange({ ...props.value, font_scale }) }), _jsx("label", { className: "af-appearance__label", children: "Header size" }), _jsx(HeaderDensitySelect, { value: props.value.header_density, onChange: (header_density) => props.onChange({ ...props.value, header_density }) })] }), _jsx("div", { className: "af-appearance__actions", children: _jsx("button", { type: "button", className: "af-appearance__close", onClick: props.onClose, children: "Close" }) })] }) }));
}
