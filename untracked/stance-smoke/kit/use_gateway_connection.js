...
export function useGatewayConnection(options) {
    ...
    const episodeOrigin = useRef("none");
    ...
    const applyStatus = useCallback((s) => {
        ...
        if (nowConnected) {
            ...
            const policyAllows = variantRef.current === "blocking" || episodeOrigin.current !== "mid" || autoOpenMidSessionRef.current;
            if (!dismissedThisEpisode.current && policyAllows)
                setModalOpen(true);
        }
        if (s !== null)
