...
type StanceTargets = {
  breath: number; // 0..1 effort share (0 when unreadable)
  effortNull: boolean;
  strokes: { fail: boolean; verify: boolean }[];
  actNull: boolean;
  fils: number; // filament count (capped)
  attOverflow: number;
  attNull: boolean;
  buds: number; // formed count (capped)
  budsOverflow: number;
  align: number; // 0..1 rigor share (1 = straight); null → 1 with rigNull
  rigNull: boolean;
  actOverflow: number;
};

function targetsFrom(facts: ConductFacts | null | undefined, tools: ConductToolCall[], axes: ConductAxis[]): StanceTargets {
  const byId = (id: string) => axes.find((a) => a.id === id);
...
export function AfConductGauge(props: AfConductGaugeProps): React.ReactElement {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const axesRef = useRef<ConductAxis[]>([]);
  const targetsRef = useRef<StanceTargets>({
  ...
  const shownRef = useRef({ breath: 0, align: 1, strokeGrow: [] as number[], filGrow: [] as number[], budGrow: [] as number[] });
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      for (let i = 0; i < s.strokeGrow.length; i++) s.strokeGrow[i] += (1 - s.strokeGrow[i]) * kg;
      for (let i = 0; i < s.filGrow.length; i++) s.filGrow[i] += (1 - s.filGrow[i]) * kg;
      for (let i = 0; i < s.budGrow.length; i++) s.budGrow[i] += (1 - s.budGrow[i]) * kg;
  ...
  return (
    <div className={["af-conduct-gauge", props.className || ""].filter(Boolean).join(" ")}>
      <canvas
        ref={canvasRef}
        className="af-conduct-gauge__canvas"
        style={{ width: size, height: size }}
        role="img"
        aria-label="Conduct stance: effort as breath, tool calls as strokes, memories as roots, rigor as alignment — mechanical facts, session-relative"
      />
      {props.note ? <div className="af-conduct-gauge__note">{props.note}</div> : null}
    </div>
  );
}
...
function drawStance(
  ...
  h: number,
  t: StanceTargets,
  s: { breath: number; align: number; strokeGrow: number[]; filGrow: number[]; budGrow: number[] },
  time: number,
  labels: boolean,
): void {
  ...
  if (t.actOverflow > 0) ctx.fillText(`+${t.actOverflow}`, cx + R * 0.3, yTop + spineH * 0.16);
  if (t.attOverflow > 0) ctx.fillText(`+${t.attOverflow}`, cx - R * 0.4, yBase - spineH * 0.05);
  if (t.budsOverflow > 0) ctx.fillText(`+${t.budsOverflow}`, cx - R * 0.42, yBase + R * 0.05);
  if (labels) {
