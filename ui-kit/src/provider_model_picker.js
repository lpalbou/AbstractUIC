...
import { ProviderModelSelect } from "./provider_model_select.js";
export function ProviderModelPicker(props) {
    ...
    useEffect(() => {
        if (isDefault || disabled || providersLoaded.current)
            return;
    ...
    const modelOptions = useMemo(() => {
        const out = [...models];
        if (modelName && !out.includes(modelName))
            out.unshift(modelName);
        return out;
    }, [models, modelName]);
    const defaultLabel = props.defaultModeLabel || "Gateway default";
    const customLabel = props.customModeLabel || "Custom";
    return (_jsxs("div", { className: `af-pmp${props.className ? ` ${props.className}` : ""}`, children: [_jsxs("div", { className: "af-pmp__seg", role: "tablist", "aria-label": "Provider mode", children: [_jsx("button", { type: "button", role: "tab", "aria-selected": isDefault, className: `af-pmp__seg-btn${isDefault ? " is-active" : ""}`, onClick: () => props.onChange({ provider: "", model: "" }), disabled: disabled, children: defaultLabel }), _jsx("button", { type: "button", role: "tab", "aria-selected": !isDefault, className: `af-pmp__seg-btn${!isDefault ? " is-active" : ""}`, onClick: () => {
                            if (isDefault)
                                props.onChange({ provider: providerOptions[0]?.name || "", model: "" });
                        }, disabled: disabled, children: customLabel })] }), isDefault ? (_jsx("div", { className: "af-pmp__hint", children: props.defaultHint || "The gateway picks its configured default provider and model for this task." })) : (_jsx(ProviderModelSelect, { provider: providerName, model: modelName, providers: providerOptions, models: modelOptions, disabled: disabled, loadingProviders: providersLoading, loadingModels: modelsLoading, providerError: providersError || undefined, modelError: modelsError || undefined, providerLabel: props.providerLabel, modelLabel: props.modelLabel, allowGatewayDefault: false, allowCustomProvider: props.allowCustom !== false, allowCustomModel: props.allowCustom !== false, onChange: (next) => props.onChange({ provider: next.provider, model: next.model }) }))] }));
}
