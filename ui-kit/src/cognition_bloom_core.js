...
export const EMOTION_REGISTERS = [
...
export function createBloomState() {
    ...
    for (const e of EMOTION_REGISTERS)
        springs[e.id] = { value: 0, velocity: 0, target: 0 };
    return { t: 0, springs };
}
...
export function setBloomTargets(state, scores) {
    for (const e of EMOTION_REGISTERS) {
        const v = scores[e.id];
...
export function tickBloom(state, dtSeconds) {
...
export function readBloom(state) {
    let vx = 0;
    let ay = 0;
    let mass = 0;
    let r = 0;
...
export function vignetteColor(reading) {
    const pleasant = reading.valence >= 0;
    const aroused = reading.arousal >= 0;
    if (pleasant && !aroused)
        return "#86c99b"; // pleasant-calm: green-gold
    if (pleasant && aroused)
        return "#e7b45a"; // pleasant-aroused: gold
    if (!pleasant && aroused)
        return "#c05a86"; // unpleasant-aroused: red-violet
 ...
 */
export function drawBloomFrame(ctx, w, h, state, opts = {}) {
...
export function effortRows(facts) {
    if (!facts)
        return [];
    const rows = [];
    for (const key of Object.keys(EFFORT_LABELS)) {
        const v = facts[key];
        if (typeof v !== "number" || !Number.isFinite(v) || v < 0)
            continue; // absent = row absent
        rows.push({
            key,
