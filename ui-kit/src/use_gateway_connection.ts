...
export function isGatewayConnected(status: GatewayConnectionState | null): boolean {
...
export function useGatewayConnection(options?: UseGatewayConnectionOptions): GatewayConnection {
  ...
  const applyStatus = useCallback((s: GatewayConnectionState | null) => {
    statusGen.current += 1;
    setStatus(s);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (statusGen.current !== gen) return;
      applyStatus(s);
    } catch {
      if (statusGen.current !== gen) return;
  ...
  const onClose = useCallback(() => {
    ...
    if (p === "disconnected") dismissedThisEpisode.current = true;
    setModalOpen(false);
  }, [variant]);
