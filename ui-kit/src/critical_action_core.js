 ...
 */
export function normalizeCriticalActionFacts(raw) {
 ...
 */
export function resolveCriticalActionGate(input) {
    if (input.busy) {
    ...
    if (!facts_ok && !input.allowDegradedProceed) {
        return {
            ...
            disabledReason: "#FALLBACK: the server did not supply blast-radius facts for this action; confirmation is disabled (facts, not vibes).",
            degraded: true,
            degradedLabel: null,
        };
    }
