...
const AVAILABILITIES: MatrixAvailability[] = [
  "granted",
  "denied",
  "structurally_unavailable",
  "trust_gated",
...
const TRUST_STATES: MatrixTrustState[] = ["attachable", "requires_review", "blocked"];
...
function validate_cell(raw: unknown, where: string): { ok: true; cell: MatrixCell } | { ok: false; reason: string } {
  if (!is_record(raw)) return { ok: false, reason: `${where}: cell is not an object` };
  const availability = String(raw.availability || "");
  if (!AVAILABILITIES.includes(availability as MatrixAvailability)) {
    return { ok: false, reason: `${where}: unknown availability "${availability}"` };
  }
  const provenance = String(raw.provenance || "");
  if (!PROVENANCES.includes(provenance as MatrixProvenance)) {
    return { ok: false, reason: `${where}: unknown provenance "${provenance}"` };
  }
  let trust_state: MatrixTrustState | undefined;
  if (availability === "trust_gated") {
    const ts = String(raw.trust_state || "");
    if (!TRUST_STATES.includes(ts as MatrixTrustState)) {
  ...
  return {
    ...
    cell: {
      ...
      provenance: provenance as MatrixProvenance,
      availability: availability as MatrixAvailability,
 ...
 */
export function validateMatrixPayload(raw: unknown): MatrixValidation {
  if (!is_record(raw)) return { ok: false, reason: "payload is not an object" };
  ...
  for (const p of raw.phases) {
    ...
    const id = p.id.trim();
    if (RESERVED_IDS.has(id)) return { ok: false, reason: `reserved phase id "${id}"` };
    if (phase_ids.has(id)) return { ok: false, reason: `duplicate phase id "${id}"` };
    phase_ids.add(id);
    phases.push({
      id,
  ...
  for (const s of raw.sections) {
    ...
    const sid = s.id.trim();
    if (section_ids.has(sid)) return { ok: false, reason: `duplicate section id "${sid}"` };
    section_ids.add(sid);
    if (!Array.isArray(s.items)) return { ok: false, reason: `section "${sid}" has no items array` };
    ...
    for (const it of s.items) {
      ...
      if (item_ids.has(iid)) return { ok: false, reason: `section "${sid}": duplicate item id "${iid}"` };
      item_ids.add(iid);
      ...
      const cells: Record<string, MatrixCell> = Object.create(null);
...
export type MatrixCellControl =
  | { kind: "absent" }
  | { kind: "blocked"; blocked_kind: "structural" | "trust" }
  | { kind: "approval" }
  | { kind: "tristate"; options: MatrixCellOp[]; active_op: MatrixCellOp };
...
function pending_for(
  ...
  for (const p of patches) {
    if (p.section === section && p.item === item && p.phase === phase) return p.op;
  }
  return null;
}
 ...
 */
export function resolveCellView(
  ...
  const structurally_blocked =
    cell.availability === "structurally_unavailable" ||
    (cell.availability === "trust_gated" && cell.trust_state === "blocked");

  let effective: boolean | null = cell.resolved_value;
  if (pending === "grant") effective = true;
  else if (pending === "deny") effective = false;
  ...
  const approval_required =
    cell.availability === "trust_gated" && cell.trust_state === "requires_review";
  const effective_value = structurally_blocked ? false : effective;

  let control: MatrixCellControl;
  if (structurally_blocked) {
    ...
    control = {
      ...
      active_op:
        pending !== null ? pending : cell.assigned ? (cell.resolved_value ? "grant" : "deny") : "clear",
    };
  }
...
export function applyCellAction(
  payload: MatrixPayload,
  patches: MatrixCellPatch[],
  patch: MatrixCellPatch
): MatrixCellPatch[] {
  const view = resolveCellView(payload, patches, patch.section, patch.item, patch.phase);
  if (!view.cell || !view.interactive) return patches;

  const rest = patches.filter(
    (p) => !(p.section === patch.section && p.item === patch.item && p.phase === patch.phase)
  );
...
export function reconcilePatches(
  ...
  for (const p of patches) {
    ...
    if (!view.cell || !view.interactive) continue;
    out.push(p);
  }
  return out;
}
 ...
 */
export function serializeCellPatches(
  payload: MatrixPayload,
  patches: MatrixCellPatch[]
): MatrixPatchDocument {
  return {
